...
# set the name of the file that will be created of pictures from the bacterial features. Don't use spaces.
file_descripter <- 'bac_lipids'
# set TRUE for mode that experiment was measured in.
pos_mode <- FALSE
neg_mode <- TRUE
# set the pattern as a regular expression to find specific features.
pattern <- '\\d*\\.[4567]\\d*'
...
date <- Sys.Date()
file_name <- paste0(date, '_', file_descripter, '_nt_', round(noise_threshhold), '.pdf')
...
dev.off()
rm(p, uz, bac_matches, backies, inten)
...
adduct_prob <- function(tab, occ_list, mzlist, pos_ions, neg_ions, pos_mode, neg_mode) {
  ...
  for (pos in seq_len(nrow(tab))) {
    if (sum(!is.na(tab[pos, ])) != 0) {                  # pos 2439
      # tab occurences
      nona <- which(!is.na(unname(tab_no_tol[pos, ])))   # nona 7, 21
      most <- findInterval(nona, slices, left.open = TRUE)+1  # most 2, 6
      # in other rows occurences as
      if (!is.null(occ_list[[pos]])) {
        ox <- (unname(which(occ_list[[pos]]== mzlist[pos], arr.ind = TRUE)[,2])+1)/2  # 4  4  8 16 17 19
        ox_mzs <- rownames(which(occ_list[[pos]]== mzlist[pos], arr.ind = TRUE))
        occin <- sapply(ox, function(x) unname(which(ma == x, arr.ind = TRUE)[,2])) +1 # 5 5 4 5 6 6
      }
      else {occin <- c()}
...
proof_adducts <- function(probs, tab, occ_list) {
  ...
  selec <- as.numeric(mzs[unname(unique_adducts)])
  print(selec)
  clear <- probs[(probs[,1] %in% selec),]
  return(clear)
}
print(proof_adducts(adduct_probabilities,net,occurence_list))
