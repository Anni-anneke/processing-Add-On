...
pixel_filter_nb <- function(experiment, min_pixel, proxim_range, chunked=FALSE, chunk_amount = 4L){
  FUN <- function(experiment, sp) {
    ...
    ### Now features that were previously discarded are checked if any of these "cluster" by checking the distance
    ...
    distance_test_bool <-  vapply(seq_along(n0_features_position), function(i) {
      ...
      return(any(dist(xy_cord[idx, ]) <= proxim_range))},logical(1) )
...
adduct_finder <- function(mse, name_mz_table, tol, adduct = "prot") {

  pos_adducts <- list("prot" = 1.0078250319, "sod" = 	22.98976928, "pot" =  38.96370648)
  num_adducts <- length(pos_adducts)
  if (class(mse) == "numeric") {
    mse_mz <- mse
  } else {
    mse_mz <- mse@featureData$mz
  }
  list_fit <- function(i, uniques, nmz_integer, mse_integer, mse_mz){
    place_holder1 <- logical(length(uniques))
    place_holder2 <- logical(length(uniques))
    place_holder1[uniques %in% nmz_integer[[i]]] <- mse_integer[[i]]
    place_holder1[place_holder1 == 0] <- NA
    place_holder2[uniques %in% nmz_integer[[i]]] <- mse_mz[[i]]
    place_holder2[place_holder2 == 0] <- NA

    return(list(int = place_holder1, mz =place_holder2))
  }


  if (adduct == "all") {
    mz_values <- sapply(1:num_adducts, function(i) name_mz_table$mz + pos_adducts[[i]])
  } else {
    mz_values <- name_mz_table$mz + pos_adducts[[adduct]]
  }

  if (any(class(mz_values) == "numeric")) {

    matches <- find_mz_matches_LEO(mz_values, mse_mz, tol)
    nmz_integer <- which(matches$matches == TRUE)
    nmz_mass <- name_mz_table$mz[nmz_integer] + pos_adducts[[adduct]]
    mse_integer <- matches$int[!is.na(matches$int)]

    result <- cbind(name_mz_table[nmz_integer,], mse_mzs = mse_mz[mse_integer], mse_integer = mse_integer)

    return(result)

  } else if (ncol(mz_values) == num_adducts) {

    matches <- sapply(1:num_adducts,function(i) find_mz_matches_LEO(mz_values[,i], mse_mz, tol))
    nmz_integer <- sapply(1:num_adducts,function(i) which(matches[,i]$matches))
    nmz_mass <- sapply(1:length(nmz_integer), function(i) name_mz_table$mz[nmz_integer[[i]]] + pos_adducts[[i]])
    mse_integer <- sapply(1:num_adducts,function(i) matches[,i]$int[!is.na(matches[,i]$int)])
    mse_mz <- sapply(1: num_adducts, function(i) mse_mz[mse_integer[[i]]])

    uniques <- sort(unique(unlist(nmz_integer)))
    result <- sapply(1:num_adducts, function(i) list_fit(i, uniques, nmz_integer, mse_integer, mse_mz))
    results <- data.frame(1:length(uniques))
    for(n in 1:num_adducts){
      a <- cbind(result[,n]$mz, result[,n]$int)
      results <-  cbind(results, a)
    }
    results[,1] <- rm()
    results <- cbind(name_mz_table[uniques,] , results)
    result_names <- sapply(1:num_adducts, function(i) c(paste0(names(pos_adducts[i]), "_mz"), paste0(names(pos_adducts[i]), "_int")))
    result_names <- c("metabolit","exact_mz", result_names[,1:num_adducts])
...
pixel_filter_chunks <- function(experiment, min_count_pix, chunked=FALSE, chunk_amount = 4L) {
  cat('removing pixel \n')

  FUN <- function(experiment) {
    int_mat <- round(experiment@spectraData$intensity[],4)
    # calculating max intensity value of a feature across all spectra.
    row_sums <- rowSums(int_mat>0)
    proof <- row_sums <= min_count_pix
    # comparing whether max value divided by all spectra is equal to the mean. If so, that feature only occurs in one pixel.
    mse_count <- subsetFeatures(experiment, proof) # only contains features that occur in one pixel
    tossed <- !(mz(experiment) %in% mz(mse_count))
    return(tossed)
  }
  # To chunk or not to chunk
  ...
  experiment <- subsetFeatures(experiment, vec) # removes all pixel smaller x
  return(experiment)

}

### adduct finder functions ###
...
.occurences <- function(mz_list, tab) {
  net_r  <- round(tab, 4)
  row_hits_mat <- sapply(mz_list, function(mz) {
  ...
  for (pos in seq_len(nrow(tab))) {
    if (sum(!is.na(tab[pos, ])) != 0) {
      ...
      if (!is.null(occ_list[[pos]])) {
        ...
        ox_mzs <- rownames(which(occ_list[[pos]]== mzlist[pos], arr.ind = TRUE))
        occin <- sapply(ox, function(x) unname(which(ma == x, arr.ind = TRUE)[,2])) +1
      }
      else {occin <- c()}

      if (length(most)+length(occin) == 0) {
        result <- rbind(result, data.frame(mz= mzlist[pos], ion = no_info, prob = 0, relations = NA))
        next
      }
      ...
      for (pi in posion) {
        ...
        if (!is.null(nona_filt)) {
          nona_mzs <- unname(tab_no_tol[pos, nona_filt])
        } else {nona_mzs <- c()}

        if (length(occin_filt)>0) {
          occ_mzs <- as.numeric(ox_mzs[occin_filt])
        }
        else {occ_mzs <- c()}

        related_mzs <- c(nona_mzs, occ_mzs)
...
# not finished
proof_adducts <- function(probs, tab, occ_list) {
  unique_adducts <- table(probs[,1]) ==1
  mzs <- names(unique_adducts)
  selec_unique <- as.numeric(mzs[unname(unique_adducts)])
  clear <- probs[(probs[,1] %in% selec_unique),]
  selec_relations <- sapply(clear[,4], function(x) length( x ))>2
  proof <- clear[selec_relations,]
  return(proof)
}
